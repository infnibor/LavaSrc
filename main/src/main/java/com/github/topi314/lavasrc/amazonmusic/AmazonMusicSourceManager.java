    ...
    public AudioItem loadItem(com.sedmelluq.discord.lavaplayer.player.AudioPlayerManager manager, AudioReference reference) {
        ...
        try {
            ...
            if ("albums".equals(type) && trackAsin != null) {
                ...
                // artworkUrl i isrc są przekazywane bez nadpisywania pustym stringiem
                AudioTrackInfo info = new AudioTrackInfo(
                    foundTrack.title,
                    foundTrack.artist,
                    foundTrack.duration,
                ...
                if (trackJson.audioUrl == null) {
                    trackJson.audioUrl = fetchAudioUrlFromStreamUrls(trackJson.id != null ? trackJson.id : trackId);
                }
                if (trackJson.audioUrl == null) {
